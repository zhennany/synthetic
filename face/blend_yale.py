...
root_dir = r'F:\face\Yale\Cropped'
...
for subdir in subdirs:
    im_list = glob.glob(os.path.join(root_dir,subdir,'*P00A+000E+00.pgm'))
    if len(im_list) == 0:
        continue
    all_im_list.extend(im_list)
    # im_list.sort(key = lambda x: int(os.path.basename(x)[1:os.path.basename(x).rindex('.')])) # sort with filename (id), which start from 's'

for i in range(len(all_im_list)):
    # shutil.copy(all_im_list[i], os.path.join(out_dir, '0', 'yale_'+str(i)+'.pgm'))
    
    image = cv2.imread(all_im_list[i])
    misc.imsave(os.path.join(out_dir, '0', 'yale_'+str(i)+'.png'), image)
    
    
    for j in range(i+1, len(all_im_list)):
        
        image2 = cv2.imread(all_im_list[j])

        lm1_file = all_im_list[i][:-len('.pgm')]+'_lm.npy'
        if os.path.isfile(lm1_file):
            lms1 = np.load(lm1_file)
        else:
            lms1 = do_landmark(image, detector, predictor, dodetection, plot=False)
        
        lm2_file = all_im_list[j][:-len('.pgm')]+'_lm.npy'
        ...
        bimage1, bimage2 = blend_faces(image, image2, lms1, lms2)

        misc.imsave(os.path.join(out_dir, '1', 'yale_'+'%d_%d.png'%(i,j)), bimage1)

        misc.imsave(os.path.join(out_dir_delib, '1', 'yale_'+'%d_%d.png'%(j,i)), bimage2)
        
        

## extra class 0 samples
for subdir in subdirs:
    sublist = ['_P00A+000E+20', '_P00A+000E-20', '_P00A+005E+10', '_P00A+005E-10', '_P00A+010E+00', '_P00A+010E-20', '_P00A+015E+20', '_P00A+020E+10', '_P00A+020E-10']
    x = [i for i in range(len(sublist))]
    shuffle(x)
    sublist = [sublist[i] for i in x]
    for subf in sublist[:5]:
        imfile = os.path.join(root_dir,subdir,subdir+subf+'.pgm')
        
        image = cv2.imread(imfile)
        misc.imsave(os.path.join(out_dir, '0', subdir+subf+'.png'), image)
    for subf in sublist[5:]:
        imfile = os.path.join(root_dir,subdir,subdir+subf+'.pgm')
        
        image = cv2.imread(imfile)
        misc.imsave(os.path.join(out_dir_delib, '0', subdir+subf+'.png'), image)
